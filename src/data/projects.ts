  ...
  },
  {
    ...
    detail: {
      ...
      challenges: [
        ...
        "4つの異なる美術館API + Wikidata SPARQLの統一的なデータモデル設計",
        "1リポジトリから6アプリをビルドするAppConfig + 抽象APIクラスの設計",
        "美術館APIの画像URL仕様の違い（IIIF / CDN / Wikimedia）とCORS制約への対応",
        "LINEボットの日次Cronによる6作品カルーセル配信と、Webhookによる作家名検索の双方向対応",
      ],
      approach: [
        "Flutter + カスタムGLSLシェーダー（lighting.frag）でスポットライトの光と影をリアルタイム描画",
        "AppConfig + ArtApi抽象クラスで美術館ごとの差異を吸収し、1リポジトリから6アプリをビルド",
        "全APIをプロキシサーバー経由にしてCORS/CDN制約を回避。画像・SPARQL・TTS等を一元管理",
        "Vercel Cronで毎朝6時に6作品をLINEカルーセル配信。作品画像はプロキシAPI経由",
        "SharedPreferencesでお気に入り・閲覧履歴をローカル管理。サーバーレスでDB不要",
      ],
      results: [
        "美術館さんぽ3種（メトロポリタン・クリーブランド・スミソニアン）、画家さんぽ2種（フェルメール・レンブラント）、特集さんぽ1種（印象派）の計6アプリを公開",
        "LINEボットで毎朝6時に6作品のカルーセルを自動配信",
        "GPUシェーダーによるスポットライト演出をブラウザで実現",
        "1リポジトリ・共通設計から6アプリをビルドする拡張性の高いアーキテクチャ",
        "新シリーズ追加はconfig + API + エントリポイントの3ファイルで完結",
      ],
      techDetail:
        "Flutter (Dart) + Material 3 + カスタムGLSLフラグメントシェーダー。API: Met Museum / AIC / Cleveland / Smithsonian / Wikidata SPARQL。プロキシ: Next.js（CORS回避・APIキー管理・画像配信・TTS）。LINEボット: Next.js + Vercel Cron（毎朝6時に6作品カルーセル配信）。デプロイ: Vercel（6アプリ + Bot）。データ: SharedPreferences（ローカル）。PWA対応。",
    },
  },
  {
    slug: "estimate-ai",
    ...
    detail: {
      ...
      challenges: [
        ...
        "工程ごとのボトルネック・リスクを業界知識なしで的確に指摘する分析精度",
        "「現状→提案」の対比表示で、発注者が直感的に効果を理解できるUI設計",
        "DX提案の分析結果を見積もりツールにシームレスに引き継ぐデータ連携",
        "Gemini API無料枠の制限対策（複数キー + Claude フォールバック）",
      ],
      approach: [
        "業務記述→フロー図→現状分析→システム化提案→総合提案の5段階AIパイプライン",
        "各工程のボトルネックを担当者・ツール・リスクの3軸で構造化分析",
        "現状と提案を左右対比で表示し、優先度（高/中/低）と期待効果を付与",
        "総合提案から「この提案で見積もりを出す」ボタンで見積もりツールにデータ連携",
        "見積もり側では画面設計→技術スタック比較→機能別工数算出→エクセル出力",
      ],
      results: [
        "業務の課題を話すだけで、5分でDX提案書の原型が完成",
        "フロー図・ボトルネック分析・システム化提案・見積もりまで一気通貫",
        "エクセル出力で営業資料・社内稟議にそのまま活用可能",
        "追加情報入力で再分析が可能。対話的に提案を磨き上げられる",
      ],
      techDetail: "Next.js + TypeScript + Tailwind CSS。Gemini 2.5 Flash / Claude Haiku 4.5 のデュアルAI構成。フロー図はMermaid記法で生成しSVGレンダリング。AbortControllerによるキャンセル機能、stateキャッシュによるステップ間自由移動。エクセル出力対応。",
    },
  },
  // === 技術デモ ===
  {
