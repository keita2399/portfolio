...
export const projects: Project[] = [
  ...
  },
  {
    ...
    description:
      "重要事項説明書のPDFをアップロードするだけで、AIがリスク・注意点・確認事項を分かりやすく解説。不動産業者が顧客に説明しながら使うBtoBtoCユースケースを想定したPC/タブレット対応の2カラムUI。",
    stats: ["Gemini 2.5 Flash", "重説解析AI"],
    ...
    detail: {
      overview:
        "重要事項説明書（PDF）をGemini 2.5 Flashで解析し、再考を推奨するリスク・注意事項・問題ない項目・担当者への質問リストをJSON形式で返すWebアプリ。不動産業者が顧客への説明時にタブレットやPCで見せながら使うユースケースを想定。",
      challenges: [
        "Gemini APIのモデル名変更と新SDK（@google/genai → @langchain/google-genai）への対応",
        "アニメーション完了とAPI完了のタイミング同期（useRef dual-flag パターン）",
        "印刷用レイアウトの@media printによるCSS分離",
        "PC/タブレット両対応の2カラムレスポンシブレイアウト",
      ],
      approach: [
        "LangChainのChatGoogleGenerativeAIでPDFをマルチモーダル解析",
        "リスク・注意・OK・質問リストを構造化JSONで返すプロンプト設計",
        "左カラムでアイテム一覧・右カラムで詳細表示の2ペインUI",
        "window.print()＋print-onlyクラスによる印刷用サマリーシート",
      ],
      results: [
        "再考を推奨（最大5件）・注意（最大5件）・OK項目・質問リストを自動生成",
        "担当者への質問モーダルで全件まとめて確認可能",
        "印刷ボタン1つで顧客配布用サマリーシートを出力",
        "Google Analytics（専用プロパティ）による利用状況計測",
      ],
      techDetail:
