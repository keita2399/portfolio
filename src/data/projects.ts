  ...
  // --- 受託・自社開発 ---
  {
    ...
    detail: {
      ...
      challenges: [
        ...
        "Stripe での 3階層メンバーシップ決済と、会員ステータスのリアルタイム反映",
        "YouTube Data API クォータ管理と、チャンネル動画を会員サイトへシームレスに表示する同期パイプライン",
        "LLM・Embedding の供給元を Vertex AI（Gemini）に統一し、APIキー管理・課金経路をシンプル化",
        "動画・画像配信を Cloudflare R2 に切り出し、Vercel Pro の帯域コストを分離",
      ],
      approach: [
        "Next.js 14 App Router + Prisma + Neon Postgres でフロント / API / DB を統合",
        "LINE Login を Next.js のセッションに橋渡しし、会員プロフィールを Prisma で管理",
        "Stripe Checkout で 3階層メンバーシップ（月額サブスクリプション）を実装",
        "Vertex AI Search + Gemini Embedding で整備情報・FAQ をベクトル化、Gemini で回答生成（RAG）",
        "YouTube Data API でチャンネル動画を定期同期し、会員向けにシームレス表示",
        "Cloudflare R2 をオリジンに使い、Vercel Pro と帯域コストを分離。Vercel Pro はホスティング・SSR・Edge に集中",
      ...
      results: [
        ...
        "YouTube Data API 連携で、チャンネル動画を会員サイトにシームレス統合",
        "LLM・Embedding を Vertex AI に統一し、Cloudflare R2 で帯域コストを分離した運用構成",
        "12ヶ月保守契約付き — 企画から本番運用・継続保守まで単独で完結",
      ],
      techDetail:
        "Next.js 14 (App Router) + TypeScript + Tailwind CSS。DB: Prisma + Neon Postgres。ストレージ: Cloudflare R2（動画・画像オリジン）。AI: Vertex AI (Gemini) + Vertex AI Search + Gemini Embedding（RAG）。決済: Stripe Checkout（3階層メンバーシップ）。認証: LINE Login (OAuth 2.0)。外部API: YouTube Data API。ホスティング: Vercel Pro。クライアント: GoldenMUGI合同会社（直接契約・サービス商標登録済み）。",
    },
  },
  {
    slug: "btob-matching",
