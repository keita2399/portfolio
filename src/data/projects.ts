...
export const projects: Project[] = [
  ...
  },
  {
    ...
    description:
      "世界の美術館・博物館のコレクションをスマホで気軽に楽しめるFlutter Webアプリシリーズ。GPUシェーダーによるスポットライト演出、毎日1作品との出会い（ガチャ）、作品クイズなど、アート体験をデジタルで再現。LINEボットが毎朝おすすめ作品を届ける。美術館さんぽ・博物館さんぽ・画家さんぽ・特集さんぽの4カテゴリで7アプリを展開中。",
    ...
    detail: {
      overview:
        "メトロポリタン美術館・シカゴ美術館（AIC）・クリーブランド美術館・スミソニアン博物館のパブリックAPIとWikidata SPARQLを活用し、数十万点のコレクションをスマホで鑑賞できるWebアプリシリーズ。Flutter Webでクロスプラットフォーム対応し、カスタムGLSLフラグメントシェーダーでスポットライトの光演出を実現。LINEボットが毎朝6時に7作品のカルーセルをプッシュ配信し、作家名検索にも対応。美術館さんぽ2種・博物館さんぽ1種・画家さんぽ3種・特集さんぽ1種の計7アプリを公開中。",
      challenges: [
        "GPUフラグメントシェーダー（GLSL）によるリアルタイム光シミュレーションの実装",
        "4つの異なる美術館API + Wikidata SPARQLの統一的なデータモデル設計",
        "1リポジトリから7アプリをビルドするAppConfig + 抽象APIクラスの設計",
      ...
      approach: [
        ...
        "AppConfig + ArtApi抽象クラスで美術館ごとの差異を吸収し、1リポジトリから7アプリをビルド",
      ...
      results: [
        ...
        "LINEボットで毎朝6時に7作品のカルーセルを自動配信",
        "カスタムGLSLフラグメントシェーダーによるスポットライト演出をブラウザで実現",
        "1リポジトリ・共通設計から7アプリをビルドする拡張性の高いアーキテクチャ",
        "新シリーズ追加はconfig + API + エントリポイントの3ファイルで完結",
      ],
      techDetail:
        "Flutter (Dart) + Material 3 + カスタムGLSLフラグメントシェーダー（lighting.frag）。API: Met Museum / AIC / Cleveland / Smithsonian / Wikidata SPARQL。プロキシ: Next.js（CORS回避・APIキー管理・画像配信・TTS）。LINEボット: Next.js + Vercel Cron（毎朝6時に7作品カルーセル配信）。デプロイ: Vercel（7アプリ + Bot）。データ: SharedPreferences（ローカル）。PWA対応。",
    },
  },
  {
    slug: "estimate-ai",
